...
import { getFirestore, collection, addDoc, onSnapshot, orderBy, query, doc, setDoc, getDoc } from "firebase/firestore";
...
const db = getimport React, { useState, useEffect, useRef } from "react";
import { initializeApp } from "firebase/app";
import { getFirestore, collection, addDoc, onSnapshot, orderBy, query, doc, setDoc, getDoc } from "firebase/firestore";
...
export default function App() {
  ...
  const [writeImg, setWriteImg] = useState(null); // base64
  ...
  function handleImagePick(e) {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        // 600px 이하로 압축 (Firestore 1MB 제한 고려)
        const MAX = 600;
        ...
        setWriteImg(canvas.toDataURL("image/jpeg", 0.6));
      };
    ...
    <div style={{ ...s.app, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
      ...
      <form onSubmit={handleUnlock} style={{ display: "flex", flexDirection: "column", gap: 12, width: "100%", maxWidth: 280, padding: "0 24px", boxSizing: "border-box" }}>
        <input type="password" value={pwInput} onChange={e => { setPwInput(e.target.value); setPwError(false); }}
          placeholder="비밀번호를 입력하세요" style={{ ...s.input, textAlign: "center", letterSpacing: 4, fontSize: 16 }} autoFocus />
        ...
        <div style={{ ...s.card, marginTop: 16 }}>
          ...
          {posts.slice(0, 2).map(post => (
            <div key={post.id} style={{ borderBottom: "1px solid rgba(90,62,27,0.08)", paddingBottom: 10, marginBottom: 10 }}>
              ...
              {post.imgBase64 && <img src={post.imgBase64} alt="" style={{ width: "100%", borderRadius: 10, marginBottom: 6, maxHeight: 160, objectFit: "cover" }} />}
  ...
  if (screen === "write") return (
    <div style={s.app}>
      ...
      <div style={s.card}>
        ...
        {writeImg && <button style={{ ...s.btnSoft, fontSize: 12, marginBottom: 14 }} onClick={() => setWriteImg(null)}>사진 제거</button>}
        <label style={s.label}>오늘 어땠어요?</label>
        <textarea value={writeText} onChange={e => setWriteText(e.target.value)}
          placeholder="오늘 있었던 일, 보고 싶은 마음, 뭐든 좋아요 ☁️"
          rows={5} style={{ ...s.input, resize: "none", lineHeight: 1.7 }} />
        <button style={{ ...s.btn, width: "100%", marginTop: 16, opacity: uploading ? 0.6 : 1 }}
          onClick={handlePost} disabled={uploading || (!writeText.trim() && !writeImg)}>
          {uploading ? "저장 중... 🌿" : "기록 남기기 🌿"}
        </button>
      </div>
    </div>
  );
          ...
          {posts.map(post => {
            ...
            return (
              <div key={post.id} style={s.card}>
                ...
                {post.imgBase64 && <img src={post.imgBase64} alt="" style={{ width: "100%", borderRadius: 12, marginBottom: 10, maxHeight: 240, objectFit: "cover" }} />}
                {post.text && <>
                  <p style={{
